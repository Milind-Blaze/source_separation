...
window = scipy.signal.chebwin(n_fft, at = 42, sym = False)
window[0] = window[1]
window[-1] = window[-2]

plt.figure(figsize = (10,8))
plt.title("the window")
plt.plot(window)
plt.savefig(join(path_expt_dir,"window_plot.png"))
plt.close()
...
print(R)
savefigure("Experiment3_SNR_chebwin_42dB", "Number of components", "Average SNR over all sources", Y, labels, R, path_expt_dir)
